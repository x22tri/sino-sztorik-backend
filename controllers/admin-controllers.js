...
const { findAdditionalInfo } = require('./character-controllers')
    ...
    try {
      ...
      for (let lessonNumber = 1; lessonNumber < (lessonDatabase.length + 1); lessonNumber++) {
        ...
        for (let tier = 1; tier < 5; tier++) {
          let foundLesson = await findLessonHelper(tier, 5, lessonNumber, 100, 'admin')
...
const getAdditionalInfoAdmin = async (req, res, next) => {
  ...
  try {
    requestedCharById.character = await Character.findOne({where: {charId: req.params.charId} })
    if (!requestedCharById.character) return next(new HttpError('A karakter nem található.', 404))
  } catch (err) {
    console.log(err)
  }

  const additionalCharInfo = await findAdditionalInfo(5, 100, requestedCharById, true)
...
const updateLesson = async (req, res, next) => {
  let updatedLesson
  try {
    updatedLesson = await RevampedLesson.findOne({where: { lessonNumber: req.params.lessonId }})
    if (!updatedLesson) return next(new HttpError('A lecke nem található.', 404))
    ...
    for (let i = 1; i < req.body.tiers.length + 1; i++) {
      ...
      if (foundTier) updatedLesson['prefaceTier' + i] = foundTier.preface || null
    }
    
    await updatedLesson.save()
    res.status(200).send({message: 'Sikeres mentés!'})
  } catch (err) {
    console.log(err)
    return next(new HttpError('Hiba történt a mentéskor.', 500))
  }
}
...
const updateAllPhrasesOrOtherUses = async (req, res, next) => {
  ...
  try {
    ...
    if (!Array.isArray(req.body) || req.body.length === 0) return next(new HttpError('Érvénytelen frissítés.', 500))
    ...
    for (let i = 0; i < req.body.length; i++) {
      ...
      let foundEntryInDatabase = await table.findOne({where: { [tableId]: updatedEntry[tableId] }})
      ...
      // If the entry already exists, update all properties.
        ...
        for (const property in foundEntryInDatabase.dataValues) foundEntryInDatabase[property] = updatedEntry[property]
        ...
        let entryWithoutID = {...updatedEntry, id: undefined, [tableId]: undefined}
        let createdEntry = await table.create(entryWithoutID)
        newEntriesIDs.push({[temporaryIDSent]: createdEntry[tableId]})
      }
    }
    res.status(200).json({message: 'Sikeres mentés!', newEntriesIDs: newEntriesIDs})
...
const getAllSimilars = async (req, res, next) => {
  ...
  try {
    allSimilarsArray = await Similar.findAll({ where: {similarType: req.params.similarType}, order: ['similarGroup'] })
  } catch (err) {
    console.log(err)
  }
  ...
  try {
    ...
    for (let i = 0; i < allSimilarsArray.length; i++) {
      ...
      foundKeyword = await Character.findOne({ where: 
        {[Op.and]: [
      ...
      foundPrimitive = await Character.findOne({ where: 
        {[Op.and]: [
          {charChinese: allSimilarsArray[i].charChinese}, 
...
const updateCharacter = async (req, res, next) => {
 ...
 try {
    let charInDatabase = await Character.findOne({where: { charId: req.params.charId }})
    const char = {...req.body}
    ...
    try {story = JSON.parse(char.story)} catch (err) {}
    if (!story || !story[0].children || !Array.isArray(story[0].children)) {
      return next(new HttpError('A szerverre beérkező történetnek érvénytelen a szerkezete.', 400))
    } else {
      ...
      for (let i = 0; i < storyLeafArray.length; i++) {
        ...
        else if (currentSegment.keyword) convertedStory = convertedStory.concat(`{k|${currentSegment.text}}`)
        else if (currentSegment.primitive) convertedStory = convertedStory.concat(`{p|${currentSegment.text}}`)
        else if (currentSegment.constituent) convertedStory = convertedStory.concat(`{${currentSegment.constituent}|${currentSegment.text}}`)
    ...
    const writeSimilars = async (similarType) => {
      ...
      if (!attributeName) return new HttpError('Érvénytelen argumentum lett megadva a writeSimilars függvényhez.', 400)
      if (!Array.isArray(char[attributeName])) return new HttpError('A szerverre beérkező hasonló karakterek listájának érvénytelen a szerkezete.', 400)

      // If the current character is already in the database, we'll add to its group.
      let foundCurrentCharInSimilar = await Similar.findOne({where: { charChinese: char.charChinese, similarType: similarType }, raw: true})

      for (let i = 0; i < char[attributeName].length; i++) {        
        if (foundCurrentCharInSimilar) {
          await Similar.findOrCreate({where: {[Op.and]: [{charChinese: char[attributeName][i]}, {similarType: similarType}]}, 
            defaults: 
            {charChinese: char[attributeName][i], similarType: similarType, similarGroup: foundCurrentCharInSimilar.similarGroup}})
            
        // If the character is not in the database yet, create a new similarGroup.
        } else {
          // Find the lowest unused group number.
          let similarGroups = await Similar.findAll({attributes: ['similarGroup'], raw: true})
          let similarGroupArray = similarGroups.map(groupObject => groupObject.similarGroup)

          similarGroupArray.sort((a, b) => a - b) // Sort in numerical order.
          let newSimilarGroupNumber = 0
          for (j = 0;  j < similarGroupArray.length; j++) {if (similarGroupArray[j] === j) newSimilarGroupNumber = j; break}
          if (newSimilarGroupNumber === 0) newSimilarGroupNumber = similarGroupArray[similarGroupArray.length - 1] + 1
          ...
          foundCurrentCharInSimilar = await Similar.create({similarGroup: newSimilarGroupNumber, charChinese: char.charChinese, similarType: similarType})
          await Similar.create({similarGroup: newSimilarGroupNumber, charChinese: char[attributeName][i], similarType: similarType})
        }
      }

      if (foundCurrentCharInSimilar) {
        // Check for any similars that were removed in the admin screen, and remove them from the database.
        let similarsInDatabase = await Similar.findAll({where: 
          { similarGroup: foundCurrentCharInSimilar.similarGroup, charChinese: {[Op.not]: char.charChinese}, similarType: similarType }
          , raw: true})

        let destroyedCharsCount = 0
        for (k = 0;  k < similarsInDatabase.length; k++) {
          if (!char[attributeName].includes(similarsInDatabase[k].charChinese)) {
            destroyedCharsCount++
            await Similar.destroy({where: {similarId: similarsInDatabase[k].similarId}})
          }
        }
        // Remove the current char from the similars database if it now has no similars.
        if (destroyedCharsCount === similarsInDatabase.length) await Similar.destroy({where: {charChinese: char.charChinese, similarType: similarType }})
      }
    }
    ...
    const writePhrasesOrOtherUses = async (attributeName) => {
      if (attributeName !== 'phrases' && attributeName !== 'otherUses') return new HttpError('Érvénytelen argumentum lett megadva a writePhrasesOrOtherUses függvényhez.', 400)
      if (!Array.isArray(char[attributeName])) return new HttpError('A szerverre beérkező kifejezések vagy egyéb használatok listájának érvénytelen a szerkezete.', 400)
      ...
      if (attributeName === 'phrases') {database = Phrase; idName = 'phraseId'; charColumnName = 'phraseChinese'} 
      else {database = OtherUse; idName = 'otherUseId'; charColumnName = 'charChinese'}

      // Add all new phrases and otherUses.
      for (let i = 0; i < char[attributeName].length; i++) {
        let currentPhraseOrOtherUse = char[attributeName][i]
        if (attributeName === 'otherUses') currentPhraseOrOtherUse.charChinese = char.charChinese
        // If the entry already exists in the database (i.e. has an ID), update its properties.
        if (currentPhraseOrOtherUse[idName]) {
          let foundElementById = await database.findOne({where: {[idName]: currentPhraseOrOtherUse[idName]}})
          if (foundElementById) {
            for (const property in currentPhraseOrOtherUse) foundElementById[property] = currentPhraseOrOtherUse[property]
      ...
      const allIDsInRequest = char[attributeName].map(element => element[idName])
      await database.destroy({where: {[charColumnName]: {[Op.like]: `%${char.charChinese}%`}, [idName]: {[Op.notIn]: allIDsInRequest}}, raw: true})
    }
    ...
    console.log(err)
    return next(new HttpError('Nem sikerült elmenteni a karaktert.', 500))
  }
}

exports.getAllLessons = getAllLessons
exports.getAdditionalInfoAdmin = getAdditionalInfoAdmin
exports.updateLesson = updateLesson
exports.getAllPhrases = getAllPhrases
exports.updateAllPhrasesOrOtherUses = updateAllPhrasesOrOtherUses
exports.getAllOtherUses = getAllOtherUses
exports.getAllSimilars = getAllSimilars
